        ...
        <div>
            ...
            <form onSubmit={handleSubmit(handleSubmit(handleRegister))} className="bg-white w-96 mx-auto text-black rounded-xl px-7 py-10">
                ...
                <div>
                    ...
                    {errors?.firstName && <p className="label-text text-white">{errors?.firstName?.message}</p> }
                </div>
                ...
                <div>
                    ...
                    <input {...register("email", {required : "Email is required for sign up"})} placeholder="Email" type="email" className="input input-bordered input-md w-full " />

                    {errors?.email && <p className="label-text text-white">{errors?.email?.message}</p> }
                </div>
